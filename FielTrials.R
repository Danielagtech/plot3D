### En un principio pense el hacerlo modular a la app, en diferentes scripts, para 
## una bueno legibilidad del codigo y la mejor experencia en las correcciones futuras. 
## Decidi solamente separar las funciones ya que son las que mas cambios se aplican a medidad
## que descrubo bugs o quiero hacer mejoras, el resto del codigo es exclusivamente visualizacion de datos.

##Daniel Perez (dperez5@mail.austral.edu.ar)
...
source("Funciones/fclustIndex.R")
source("Funciones/maximo_densidad.R")
source("Funciones/maximo.R")
source("Funciones/appy_fuzzy.R")
source("Funciones/calcularFrecuencias.R")
source("Funciones/calcula_productor.R")


data <- read.csv("CSV/foliaresDT.csv")

lotes <- c("Guayaibi", "Pertenencia")


coeficientes_csv <- read_csv("CSV/coeficientes.csv")
...
ui <- dashboardPage(
  dashboardHeader(title = "FieldTrials AID"),
  
  dashboardSidebar(
    sidebarMenu(
      menuItem("AID", tabName = "tab_AID", icon = icon("info-circle")),
  ...
  tabItems(
    # Tab Red Experta
    tabItem(tabName = "tab_AID",
            fluidPage(
              titlePanel("Análisis Inteligente de Datos"),
              ...
              div(
                ...
                p("Esta aplicación Shiny ha sido desarrollada para ayudar a técnicos y productores a visualizar de manera eficiente 
              los datos provenientes de ensayos de campo, evaluaciones de híbridos y escenarios agronómicos. Proporciona herramientas 
              para identificar patrones clave, optimizar decisiones y maximizar el rendimiento agronómico en diferentes ambientes."),
              
              
              p("Los usuarios pueden visualizar curvas de respuesta probable, realizar simulaciones económicas, 
              comparar resultados, y obtener métricas de eficiencia agronómica. Estos análisis están orientados a facilitar 
              la toma de decisiones basada en datos, permitiendo optimizar la relación costo-beneficio para maximizar el margen bruto por hectárea."),
              
              
              p("Las principales funcionalidades de esta aplicación incluyen:"),
              tags$ul(
                tags$li("Análisis de Curvas de Respuesta: Modelos NxDxA para evaluar el impacto de la densidad, fertilización y ambiente."),
                tags$li("Analisis de curvas respuesta: Modelos de Densidad por ambientes (DxA)."),
                tags$li("Mapas de Prescripción y Cosecha: Comparación geoespacial de ambientes."),
                tags$li("Foliares: Prueba de insumos de aplicación foliar en soja y maiz en diferentes regiones del país.")
              ),
              
            
              p("¡Comenza! consultas: dperez5@mail.austral.edu.ar")
              )
            )
    ),
    # Tab Modelos
...
server <- function(input, output, session) {
  ...
  valores_resumen <- reactive({
    ...
    for (hibrido in hibridos) {
      for (ambiente in ambientes) {
        ...
        field <- unique(coefs$Field)             
        fertilizante <- unique(coefs$Fertilizante) 
  ...
  output$tabla_resumen_densidad <- renderDT({
    ...
    datatable(resumen,
      ...
      formatStyle(
        columns = colnames(resumen), 
        background = styleColorBar(resumen$`Margen Bruto Máximo (USD)`, 'lightblue') 
      )
  })
  ...
  observe({
    ...
    archivo <- switch(lote,
                      "Guayaibi" = "SHP/Guayaibi.shp",
                      "Pertenencia" = "SHP/Pertenencia.shp",
                      stop("Lote no válido"))
    ...
    output$p_medias <- renderPlot({
      resumen <- datos %>%
        ...
        summarise(RindePromedio = mean(Rinde, na.rm = TRUE))  
      
      ggplot(resumen, aes(x = cluster_cosecha, y = RindePromedio, fill = cluster_cosecha)) +
        geom_bar(stat = "identity", position = position_dodge(), width = 0.7) + 
        geom_text(aes(label = round(RindePromedio, 1)), 
